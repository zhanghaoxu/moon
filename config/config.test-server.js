...
'use strict';
const testServerMysqlConfig = require('../database/config.json')['test-server'];
/**
...
module.exports = {
  sequelize: testServerMysqlConfig,

  mysql: {
    // 单数据库信息配置
    client: {
      // host
      host: testServerMysqlConfig.host,
      // 端口号
      port: testServerMysqlConfig.port,
      // 用户名
      user: testServerMysqlConfig.username,
      // 密码
      password: testServerMysqlConfig.password,
      // 数据库名
      database: testServerMysqlConfig.database,
    },
    // 是否加载到 app 上，默认开启
    app: true,
    // 是否加载到 agent 上，默认关闭
