...
'use strict';
const preServerMysqlConfig = require('../database/config.json')['dev-server'];
/**
...
module.exports = {
  sequelize: preServerMysqlConfig,

  mysql: {
    // 单数据库信息配置
    client: {
      // host
      host: preServerMysqlConfig.host,
      // 端口号
      port: preServerMysqlConfig.port,
      // 用户名
      user: preServerMysqlConfig.username,
      // 密码
      password: preServerMysqlConfig.password,
      // 数据库名
      database: preServerMysqlConfig.database,
    },
    // 是否加载到 app 上，默认开启
    app: true,
    // 是否加载到 agent 上，默认关闭
